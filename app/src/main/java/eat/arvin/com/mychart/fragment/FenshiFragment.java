...
public class FenshiFragment extends LineBaseFragment {
    ...
    private Handler refrushHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (isShow && !isPause) {
                fenshiView.pushingTimeSharingData(minutes.get(new Random().nextInt(minutes.size())));
            }
            if (!isPause) {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        fenshiView.setDataAndInvalidate(minutes);
    }
    ...
    private void addListener() {
        ...
        fenshiView.setmTimeSharingListener(new ChartView.TimeSharingListener() {
...
//                ats_ll_container.setVisibility(View.INVISIBLE);
            ...
            public void needLoadMore() {
                Toast.makeText(getContext(), "needLoadMore: 需要加载更多了", Toast.LENGTH_SHORT).show();
                loadMoreData();
            }
        });
    }
    ...
    private void loadMoreData() {
        fenshiView.postDelayed(new Runnable() {
            @Override
            public void run() {
                ArrayList<CMinute> list = new ArrayList<>();
                for (int i = 0; i < 15; i++) {
                    list.add(minutes.get(new Random().nextInt(minutes.size())));
                }
